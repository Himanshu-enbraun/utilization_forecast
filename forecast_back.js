...
*/
const generateJSON = async (data, PRSelection, BalanceSelection) => {
...
*/
const sendDataRequest = async (forecastReqData) => {
    const { startDate, endDate, checkboxes, PRData, BalData, LinkedBookingsOnly, env } = forecastReqData;
    ...
    // Set selected units based on checkboxes
    checkboxes.forEach(box => {
        selectedUnits[box] = true;
    });

    try {
        const response = await axios.get(`${env.ERS_API_DOMAIN}/rest/forecast`, {
            params: {
                start: startDate,
                end: endDate,
                calculateGapUsingLinkedBookings: LinkedBookingsOnly
            },
            headers: {
                Authorization: `Bearer ${env.ERS_TOKEN}`
            }
        });

        const rolesResponse = await axios.get(`${env.ERS_API_DOMAIN}/rest/roles`, {
            headers: {
                Authorization: `Bearer ${env.ERS_TOKEN}`
            }
        });
        ...
        // Generating CSV data
        return generateJSON(forecast, PRData, BalData);
    } catch (error) {
        console.error("Error fetching data:", error.message);
        throw new Error("Failed to fetch forecast and roles data");
    }
};
